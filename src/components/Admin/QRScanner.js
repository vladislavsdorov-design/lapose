...
const QRScanner = ({ onScan, onLostScan }) => {
  ...
  const [scanType, setScanType] = useState(null); // 'normal' или 'lost'
      ...
      scanner
        .start(
          ...
          (decodedText) => {
            ...
            const isLostToken = decodedText.startsWith("LOST_");
            ...
            if (scannerRef.current && scannerRef.current.isScanning) {
              scannerRef.current
                ...
                .then(() => {
                  if (isLostToken) {
                    // Это забытая курточка
                    setScanType("lost");
                    if (onLostScan) {
                      onLostScan({ token: decodedText });
                    } else {
                      // Если нет специального обработчика, передаем как есть
                      onScan({ number: decodedText, type: "lost" });
                    }
                  } else {
                    // Обычный номерок
                    setScanType("normal");
                    onScan({ number: decodedText, type: "normal" });
                  }
                  // Закрываем диалог
                  setOpen(false);
                  setLoading(false);
      ...
      >
        ...
        <DialogContent sx={{ p: 0 }}>
          <Paper sx={{ p: 2, minHeight: 500 }}>
            ...
            {scanType && (
              <Alert
                severity={scanType === "lost" ? "warning" : "info"}
                sx={{ m: 2 }}
              >
                {scanType === "lost"
                  ? "🔔 Распознан код забытой курточки"
                  : "✅ Распознан обычный номерок"}
              </Alert>
            )}
