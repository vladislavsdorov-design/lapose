...
const AdminPanel = () => {
  ...
  // ИЗМЕНЕНО: Улучшенный слушатель для выбранного билета
  useEffect(() => {
    if (!scannedTicket) {
      setTicketData(null);
      return;
    ...
    const ticketRef = ref(database, `tickets/${scannedTicket.number}`);

    const handleTicketUpdate = (snapshot) => {
      const data = snapshot.val();
      if (data) {
        console.log("Билет обновлен в реальном времени:", data);
        setTicketData(data);
      } else {
        handleCloseTicket();
      }
    ...
    const unsubscribe = onValue(ticketRef, handleTicketUpdate);
  ...
  // УДАЛЕНО: старый useEffect который конфликтовал с новым
  // useEffect(() => {
  //   if (scannedTicket && tickets[scannedTicket.number]) {
  //     setTicketData(tickets[scannedTicket.number]);
  //   }
  ...
  const handleTicketClick = (ticket) => {
    setScannedTicket({ number: ticket.number });
    // Не устанавливаем ticketData сразу, чтобы данные подтянулись из слушателя
    setTicketData(null);
  };
      ...
      <Box sx={{ mt: 4, mb: 4 }}>
        ...
        {scannedTicket && ticketData && (
          <TicketDetails
            ...
            onClose={handleCloseTicket}
            onUpdate={() => {
              // Этот вызов гарантирует, что данные обновятся
              console.log("Вызван onUpdate из TicketDetails");
              // Принудительно обновляем данные из Firebase через слушатель
              if (scannedTicket) {
                const ticketRef = ref(
                  database,
                  `tickets/${scannedTicket.number}`
                );
                onValue(
                  ticketRef,
                  (snapshot) => {
